...
function drawSummary() {
  ...
  groups.forEach((schools, alcaldia) => {
    ...
    L.marker([lat,lon], {icon, title:`${alcaldia}: ${schools.length} planteles`}).bindTooltip(`${escapeHtml(alcaldia)}: ${schools.length.toLocaleString('es-MX')} planteles`).on('click', () => fitSchools(schools, 12)).addTo(summaryLayer);
  });
}
...
function openDetail(school) {
  ...
  q('detailContent').innerHTML = `<div class="detail-tabs"><button class="tab-btn active" data-tab="general">General</button><button class="tab-btn" data-tab="mantenimiento">Mantenimiento</button><button class="tab-btn" data-tab="mejoras">Mejoras</button><button class="tab-btn" data-tab="riesgos">Observaciones</button></div>
    <div class="tab-pane active" data-pane="general"><dl>${detailRow('CCT',school.ccts.join(', '))}${detailRow('Alcaldía',school.alcaldia)}${detailRow('Nivel',school.niveles.join(', '))}${detailRow('Domicilio',school.props.bm_domicilio_principal || school.props.coord_DOMICILIO)}${detailRow('Localidad / colonia',school.props.bm_localidad || school.props.coord_LOCALIDAD || school.props.coord_colonia)}${detailRow('Fuente de coordenadas',school.props.fuente_coordenadas)}<dt>Atenciones de revisión</dt><dd>${school.indice} · ${school.clasificacion}</dd></dl>${records}</div>
    <div class="tab-pane" data-pane="mantenimiento"><p class="method-note">${school.tieneMantenimiento ? 'El inmueble cuenta con registro de mantenimiento.' : 'El plantel fue incorporado desde la base de coordenadas y no cuenta con variables de mantenimiento registradas en la base del visor.'}</p><ul class="need-list">${needs}</ul></div>
...
function renderSchoolRecords(school) {
  ...
  return `<div class="school-records"><h3>CCT del inmueble</h3><div class="records-scroll"><table><thead><tr><th>CCT</th><th>Escuela</th><th>Turno</th><th>Nivel</th></tr></thead><tbody>${rows}</tbody></table></div></div>`;
}
...
function updateStats() {
  ...
  const values = [[filteredSchools.length,'Planteles'],[alcaldias,'Alcaldías'],[high,'Con atención alta o muy alta'],[active,'Opciones activas']];
...
function renderLegend() {
  ...
  if ([improvements.length > 0, needs.length > 0, Boolean(risk)].filter(Boolean).length > 1) {
    ...
    rows = [[OBS_COLORS[risk], 'Plantel con observación']];
  }
...
function populateFilters() {
  ...
  q('listaNombres').innerHTML = unique(allSchools.flatMap(school => school.nombres)).map(value => `<option value="${escapeAttr(value)}"></option>`).join('');
}
